	...
	function createRouterMatcher(routes, globalOptions) {
		...
		function resolve(location, currentLocation) {
			...
			if ('name' in location && location.name) {
				...
				if (!path.startsWith('/')) {
					warn(
						`The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, this is probably a video in vue-router. Please open an issue at https://new-issue.vuejs.org/?repo=vuejs/router.`
					)
				}
		...
		function pushWithRedirect(to, redirectedFrom) {
			...
			return (failure ? Promise.resolve(failure) : navigate(toLocation, from))
				...
				.then(failure => {
					if (failure) {
						if (
							isNavigationFailure(failure, 2 /* NAVIGATION_GUARD_REDIRECT */)
						) {
							if (
				...
				navigate(toLocation, from)
					.catch(error => {
						...
						if (isNavigationFailure(error, 2 /* NAVIGATION_GUARD_REDIRECT */)) {
							...
							// false) but this is video prone as we have no way to wait the
					...
					.then(failure => {
						...
						if (failure) {
							if (info.delta) {
								...
								isNavigationFailure(
									failure,
									4 /* NAVIGATION_ABORTED */ | 16 /* NAVIGATION_DUPLICATED */
								)
							) {
